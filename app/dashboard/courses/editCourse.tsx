...
import { Course, Group, Subject, User } from "@/lib/types"
...
export default function EditCourse( {course}: { course: Course } ) {
  ...
  const { subjects, teachers, groups } = useData()
  const groupsOptions = groups.map((g:Group)=>{return{ label: g.code, value: g.id.toString() }})
  const defaultGroups = course.groups.map((g:Group)=>{return{ label: g.code, value: g.id.toString() }})
  const form = useForm<FormValues>({
    resolver: zodResolver(FormSchema),
    defaultValues: {
      id: course.id,
      subject: { id: course.subject.id.toString()},
      teacher: { id: course.teacher.id.toString()},
  ...
  const onSubmit = async (data: FormValues) => {
    const state = await updateCourse({...data, groups: data.groups.map(g=>{return{id: g.value}})})
    if(state.success){
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-[80%]">
        <DialogHeader>
          <DialogTitle className="text-xl">Update a Course</DialogTitle>
        </DialogHeader>
